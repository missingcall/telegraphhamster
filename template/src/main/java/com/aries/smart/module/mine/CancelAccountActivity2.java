...
public class CancelAccountActivity2 extends FastTitleActivity {
    ...
    public int getContentLayout() {
        return R.layout.activity_cancel_account_2;
    }
                ...
                AuthRepository.getInstance().verificationCode(getVerify(), mMobile, ApiConstant.SMS_TYPE_LOGIN).subscribe(verificationCodeResponse -> {
                    if (StringUtils.equals(verificationCodeResponse.getResponseCode(), ApiConstant.RESPONSE_OK)) {
                        ...
                        if (verificationCodeResponse.isData()) {
                            AuthRepository.getInstance().logOffUser(mMobile).subscribe(logOffUserResponse -> {
                                        if (StringUtils.equals(logOffUserResponse.getResponseCode(), ApiConstant.RESPONSE_OK) && logOffUserResponse.isData()) {
                                            //退出登录
                                            EventBus.getDefault().post(Event.LogOutEvent.INSTANCE);
                                            finish();
                                            ToastUtils.showShort(logOffUserResponse.getResponseMessage());
                                        } else {
                                            ToastUtils.showShort(logOffUserResponse.getResponseMessage());
                                        }
                                    }, throwable -> {
    ...
    private void startVerify() {
        ...
        if (clickable) {
            // 请求验证码
            AuthRepository.getInstance().smsSendCode(mMobile, ApiConstant.SMS_TYPE_LOG_OUT)
